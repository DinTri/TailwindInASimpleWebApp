...
document.addEventListener('DOMContentLoaded', () => {
  const hash = window.location.hash // e.g. "#card"
  ...
  if (hash) {
    ...
    setTimeout(() => {
      ...
      target.classList.add('ring', 'ring-yellow-400')
      setTimeout(() => target.classList.remove('ring', 'ring-yellow-400'), 1000)
    }, 300) // wait for layout flip or delay if needed
  }
})
