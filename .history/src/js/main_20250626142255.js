...
loginForm?.addEventListener('submit', (e) => {
  ...
  if (email && password.length >= 6) {
    ...
    if (email && password.length >= 6) {
      handleLogin(email)
      loginForm.reset()
      scrollToWithOffset('card', 80) // Scroll to #card with offset for navbar
    }
  }
})
...
function scrollToCard() {
  ...
  const offsetTop = window.pageYOffset + rect.top - 80 // adjust for navbar height
  console.log('Scrolling to:', offsetTop, 'Element top:', rect.top)
