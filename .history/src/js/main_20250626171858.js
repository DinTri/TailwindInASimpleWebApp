...
if (savedEmail) {
  ...
  // If not logged in, restrict access to only Home section
  const allSections = document.querySelectorAll('section, aside')
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const protectedSections = document.getElementById('protectedSections')
  ...
  if (userEmail) {
    ...
    // Also prevent users from seeing restricted sections via hash
...
function scrollToWithOffset(id, offset = 80) {
  ...
  console.log('Scroll offset:', Math.max(0, offsetPosition))
  window.scrollTo({ top: Math.max(0, offsetPosition), behavior: 'smooth' })

  // Optional: visual highlight after scroll
  element.classList.add('ring', 'ring-yellow-400')
  setTimeout(() => element.classList.remove('ring', 'ring-yellow-400'), 1000)
}
function debounce(fn, delay) {
  let timer
  return function (...args) {
...
function scrollToWithOffset(id, offset = 80) {
  ...
  const elementPosition =
    element.getBoundingClientRect().top + window.pageYOffset
  const offsetPosition = elementPosition - offset
  console.log(`[scrollToWithOffset] Scroll to #${id}:`, offsetPosition)
...
// On page load, check localStorage and show/hide accordingly
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (userEmail) {
    ...
    welcomeMessage.textContent = `Welcome, ${capitalize(userEmail.split('@')[0])}!`
    welcomeMessage.classList.remove('hidden')
    logoutBtn.classList.remove('hidden')
    loginLink.classList.add('hidden')
  } else {
    // Not logged in: hide auth-only links
  ...
  // Now show the body after setting UI
